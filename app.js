...
async function hydrateSession() {
  ...
  try {
    ...
    setAuthStatus(`会话检查失败：${requestError.message || requestError}`, "warn");
    return;
  }
  if (error) {
    console.error(error);
...
async function handleMagicLinkSignIn(event) {
  ...
  console.info("[auth] magic-link start", { email });

  let error;

  try {
    const response = await withTimeout(
      state.supabase.auth.signInWithOtp({
        email,
        options: {
      ...
      12000,
      "登录链接发送超时，请检查当前网络是否能访问 Supabase。"
    );
    error = response.error;
    console.info("[auth] magic-link response", response);
  } catch (requestError) {
    refs.sendLinkBtn.disabled = false;
    console.error(requestError);
    setAuthStatus(`发送失败：${requestError.message || requestError}`, "warn");
    return;
  }
...
async function handlePasswordSignIn(event) {
  ...
  console.info("[auth] password sign-in start", { email });
  ...
  try {
    ...
    error = response.error;
    console.info("[auth] password sign-in response", response);
  } catch (requestError) {
    refs.signInBtn.disabled = false;
    console.error(requestError);
    setAuthStatus(`登录失败：${requestError.message || requestError}`, "warn");
    return;
  }
...
async function handlePasswordSignUp() {
  ...
  console.info("[auth] sign-up start", { email });
  ...
  try {
    ...
    data = response.data;
    error = response.error;
    console.info("[auth] sign-up response", response);
  } catch (requestError) {
    refs.signUpBtn.disabled = false;
    console.error(requestError);
    setAuthStatus(`注册失败：${requestError.message || requestError}`, "warn");
    return;
  }
...
async function loadRecords(options = {}) {
  ...
  try {
    ...
    updateSyncCard();
    console.error(requestError);
    setAuthStatus(`同步失败：${requestError.message || requestError}`, "warn");
    return;
  }
  ...
  if (options.importLegacy && records.length === 0) {
    ...
    if (imported > 0) {
      const secondPass = await withTimeout(
        state.supabase
          .from("ledger_entries")
          .select(LEDGER_SELECT_COLUMNS)
          .order("occurred_on", { ascending: false })
