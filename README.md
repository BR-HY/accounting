...
1. 在 Supabase 创建项目。
2. 打开 SQL Editor，执行 `supabase/schema.sql`。脚本会创建新的账本表 `public.natural_ledger_records`，并把它加入 Supabase Realtime 发布。
...
- 这版已加入实时同步、跨标签页同步和“刷新数据”按钮；如果你是从旧版本升级，请重新执行一次 `supabase/schema.sql`，确保 Realtime 已对 `public.natural_ledger_records` 生效
